...
### 랜덤 추출
sample(data,count,replace=T)
- data:선택할 데이터 범위
- count:랜덤 추출 반복 회수
- replace:중복 추출 가능 여부. T이면 중복 추출 가능

```R
sample(1:6,10,replace=T)
#[1] 4 3 2 6 6 3 3 4 3 6
```
...
### 외부 패키지(라이브러리) 가져오기
외부 패키지를 가져와서 사용이 가능하다.

인스톨 명령어와, 라이브러리 명령어만 실행하면 바로 사용가능하다. 자바에 비해 매우 간편하다.
```R
# openxlsx 패키지 설치
